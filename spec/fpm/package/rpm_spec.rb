  ...
  describe "#output", :if => program_exists?("rpmbuild") do
    context "package attributes" do
      ...
      it "should have the correct 'verify' script" do
        insist { @rpm.attributes[:rpm_verifyscript] } == "example verifyscript"
        insist { @rpm.tags[:verifyprog] } == "/bin/sh"
      end

      it "should have the correct 'pretrans' script" do
        insist { @rpm.attributes[:rpm_pretrans] } == "example pretrans"
        insist { @rpm.tags[:verifyprog] } == "/bin/sh"
      end

      it "should have the correct 'posttrans' script" do
        insist { @rpm.attributes[:rpm_pretrans] } == "example posttrans"
        insist { @rpm.tags[:verifyprog] } == "/bin/sh"
      end

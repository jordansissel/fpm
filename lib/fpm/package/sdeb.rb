...
# Support for debian source package
#
# This class supports only output of packages.
class FPM::Package::SDEB < FPM::Package
  ...
  option "--custom-dsc", "FILEPATH",
    "Custom version of the Debian dsc file." do |dsc|
    File.expand_path(dsc)
  end

  option "--custom-rules", "FILEPATH",
    "Custom version of the Debian rules file." do |rules|
    File.expand_path(rules)
  end
  ...
  def initialize(*args)
    super(*args)
    attributes[:sdeb_priority] = "optional"
  end # def initialize
  ...
  def output(output_path)
    ...
    attributes[:sdeb_systemd] = []
    attributes.fetch(:sdeb_systemd_list, []).each do |systemd|
      ...
      attributes[:sdeb_systemd] << name
    end

    if script?(:before_upgrade) or script?(:after_upgrade) or attributes[:sdeb_systemd].any?
      puts "Adding action files"
      if script?(:before_install) or script?(:before_upgrade)
        scripts[:before_install] = template("deb/preinst_upgrade.sh.erb").result(binding)
      end
      if script?(:before_remove) or not attributes[:sdeb_systemd].empty?
        scripts[:before_remove] = template("deb/prerm_upgrade.sh.erb").result(binding)
      end
      if script?(:after_install) or script?(:after_upgrade) or attributes[:sdeb_systemd].any?
    ...
    if attributes[:sdeb_upstream_changelog]
      File.new(dest_upstream_changelog, "wb", 0644).tap do |changelog|
        Zlib::GzipWriter.new(changelog, Zlib::BEST_COMPRESSION).tap do |changelog_gz|
            if not attributes[:source_date_epoch].nil?
              changelog_gz.mtime = attributes[:source_date_epoch].to_i
            end
            logger.info("Writing user-specified upstream changelog", :source => attributes[:sdeb_upstream_changelog])
            File.new(attributes[:sdeb_upstream_changelog]).tap do |fd|
              chunk = nil
              # Ruby 1.8.7 doesn't have IO#copy_stream
              changelog_gz.write(chunk) while chunk = fd.read(16384)
            end.close
    ...
    attributes.fetch(:sdeb_init_list, []).each do |init|
    ...
    attributes.fetch(:sdeb_default_list, []).each do |default|
    ...
    attributes.fetch(:sdeb_upstart_list, []).each do |upstart|
    ...
    debiantarxz = write_debian_tarball

    dsc = write_dsc

    File.expand_path(output_path).tap do |output_path|
      ::Dir.chdir(build_path) do
        safesystem(*ar_cmd, output_path, dsc, source_archive, debiantarxz)
      end
    end

    # if a PACKAGENAME.changes file is to be created
    if self.attributes[:sdeb_generate_changes?]
      distribution = self.attributes[:sdeb_dist]
  ...
  def converted_from(origin)
    ...
    if origin == FPM::Package::Deb
      ...
      if File.exists?(changelog_path)
        ...
        attributes[:sdeb_changelog] = build_path("deb_changelog")
        File.open(attributes[:sdeb_changelog], "w") do |deb_changelog|
    ...
    if origin == FPM::Package::Deb
      ...
      if File.exists?(changelog_path)
        ...
        attributes[:sdeb_upstream_changelog] = build_path("deb_upstream_changelog")
        File.open(attributes[:sdeb_upstream_changelog], "w") do |deb_upstream_changelog|
          Zlib::GzipReader.open(changelog_path) do |gz|
  ...
  def fix_dependency(dep)
    ...
    if dep =~ /\(~>/
      ...
      self.attributes[:sdeb_breaks] ||= []
      self.attributes[:sdeb_breaks] << dep.gsub(/!=/,"=")
      return []
    elsif (m = dep.match(/(\S+)\s+\(= (.+)\)/)) and
        self.attributes[:sdeb_ignore_iteration_in_dependencies?]
  ...
  def write_dsc
    staging_path("#{name}.dsc").tap do |dsc|
      if attributes[:sdeb_custom_dsc]
        logger.debug("Using '#{attributes[:sdeb_custom_dsc]}' template for the .dsc file")
        dsc_data = File.read(attributes[:sdeb_custom_dsc])
      else
        logger.debug("Using 'dsc.erb' template for the dsc file")
        dsc_data = template("sdeb/dsc.erb").result(binding)
      end

      logger.debug("Writing dsc file", :path => dsc)
      File.write(dsc, dsc_data)
      File.chmod(0644, dsc)
    end
  end # def write_dsc

  def dsc_files(algorithm="md5")
    case algorithm
    when "md5"
    ...
    when "sha256"
      ...
      raise "Invalid digest algorithm '#{algorithm}'. Expected one of 'md5', 'sha1', 'sha256'"
    end

    dsc_files = []
    ::Dir.glob("#{build_path}/*") do |f|
      checksum = algorithm.send('file', f).hexdigest
      size = File.size(f)
      dsc_files << "#{checksum} #{size} #{File.basename(f)}"
    end
    return dsc_files
  end # def dsc_files

  def write_debian_tarball
    write_rules
    write_control
    write_shlibs
    write_scripts
    ...
    debiantarxz = build_path("debian.tar.xz")

    # Make the debian.tar.xz
    logger.info("Creating", :path => debiantarxz, :from => debian_dir)

    args = [ tar_cmd, "-C", File.dirname(debian_dir),  "-Jcf", debiantarxz,
             "--owner=0", "--group=0", "--numeric-owner", File.basename(debian_dir) ]
    ...
    logger.debug("Removing no longer needed debian dir", :path => debian_dir)
    FileUtils.rm_r(debian_dir)
  end # def write_debian_tarball

  def write_rules
    debian_dir("rules").tap do |rules|
      if attributes[:sdeb_custom_rules]
        logger.debug("Using '#{attributes[:sdeb_custom_rules]}' template for the rules file")
        rules_data = File.read(attributes[:sdeb_custom_rules])
      else
        logger.debug("Using 'rules.erb' template for the control file")
        rules_data = template("sdeb/rules.erb").result(binding)
      end

      logger.debug("Writing rules file", :path => rules)
      File.write(rules, rules_data)
      File.chmod(0755, rules)
      edit_file(rules) if attributes[:edit?]
    end
  end # def write_rules
  ...
  def write_control
    ...
    if attributes[:sdeb_installed_size].nil?
      ...
      attributes[:sdeb_installed_size] = total / 1024
    end

    # Write the control file
    debian_dir("control").tap do |control|
      if attributes[:sdeb_custom_control]
        logger.debug("Using '#{attributes[:sdeb_custom_control]}' template for the control file")
        control_data = File.read(attributes[:sdeb_custom_control])
      else
        logger.debug("Using 'deb.erb' template for the control file")
        control_data = template("sdeb/control.erb").result(binding)
      end
  ...
  def write_scripts
    SCRIPT_MAP.each do |scriptname, filename|
      ...
      debian_dir(filename).tap do |controlscript|
  ...
  def write_conffiles
    ...
    inits    = attributes.fetch(:sdeb_init_list, [])
    defaults = attributes.fetch(:sdeb_default_list, [])
    upstarts = attributes.fetch(:sdeb_upstart_list, [])
    etcfiles = []
    # Add everything in /etc
    begin
      if !attributes[:sdeb_no_default_config_files?] && File.exists?(staging_path("/etc"))
    ...
    if attributes[:sdeb_auto_config_files?]
      inits.each do |init|
        name = File.basename(init, ".init")
    ...
    debian_dir("conffiles").tap do |conffiles|
  ...
  def write_shlibs
    return unless attributes[:sdeb_shlibs]
    logger.info("Adding shlibs", :content => attributes[:sdeb_shlibs])
    File.open(debian_dir("shlibs"), "w") do |out|
      out.write(attributes[:sdeb_shlibs])
    end
    File.chmod(0644, debian_dir("shlibs"))
  end # def write_shlibs

  def write_debconf
    if attributes[:sdeb_config]
      FileUtils.cp(attributes[:sdeb_config], debian_dir("config"))
      File.chmod(0755, debian_dir("config"))
    end

    if attributes[:sdeb_templates]
      FileUtils.cp(attributes[:sdeb_templates], debian_dir("templates"))
      File.chmod(0644, debian_dir("templates"))
    end
  end # def write_debconf

  def write_meta_files
    files = attributes[:sdeb_meta_file]
    return unless files
    files.each do |fn|
      dest = debian_dir(File.basename(fn))
  ...
  def write_triggers
    lines = [['interest', :sdeb_interest],
             ['activate', :sdeb_activate],
             ['interest-noawait', :sdeb_interest_noawait],
             ['activate-noawait', :sdeb_activate_noawait],
             ].map { |label, attr|
      (attributes[attr] || []).map { |e| "#{label} #{e}\n" }
    }.flatten.join('')

    if lines.size > 0
      File.open(debian_dir("triggers"), 'a') do |f|
        f.chmod 0644
        f.write "\n" if f.size > 0
        f.write lines
      end
    end
  end # def write_triggers
  ...
  def write_md5sums
    ...
    if not md5_sums.empty?
      File.open(debian_dir("md5sums"), "w") do |out|
        md5_sums.each do |path, md5|
          out.puts "#{md5}  #{path}"
        end
      end
      File.chmod(0644, debian_dir("md5sums"))
    end
  ...
  def data_tar_flags
    data_tar_flags = []
    if attributes[:sdeb_use_file_permissions?].nil?
      if !attributes[:sdeb_user].nil?
        if attributes[:sdeb_user] == 'root'
          data_tar_flags += [ "--numeric-owner", "--owner", "0" ]
        else
          data_tar_flags += [ "--owner", attributes[:sdeb_user] ]
        end
      end

      if !attributes[:sdeb_group].nil?
        if attributes[:sdeb_group] == 'root'
          data_tar_flags += [ "--numeric-owner", "--group", "0" ]
        else
          data_tar_flags += [ "--group", attributes[:sdeb_group] ]
        end
      end
    end
  ...
  def debian_dir(path=nil)
    @debian_dir ||= staging_path("debian")
    mkdir_p(@debian_dir) unless File.directory?(@debian_dir)
    if path.nil?
      return @debian_dir
    else
      return File.join(@debian_dir, path)
    end
  end # def debian_dir

  public(:input, :output, :architecture, :name, :prefix, :version, :converted_from, :to_s, :data_tar_flags)
end # class FPM::Target::SDEB
